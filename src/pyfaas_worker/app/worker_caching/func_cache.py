...
class WorkerFunctionExecutionCache():
    class CachedResultNode():
        def __init__(self, func_id: str = None, func_positional_args: list[object] = None, func_default_args: dict[object] = None, func_result: object = None):
    ...
    def _build_key_tuple(self, func_id: str, func_positional_args: list[object], func_default_args: dict[object]) -> tuple:
        return (
            func_id,
    ...
    def add(self, func_id: str, func_positional_args: list[object], func_default_args: dict[object], func_result: object) -> None:
        if self._max_size == 0:
            # Caching is disabled
            return

        key_tuple = self._build_key_tuple(func_id, func_positional_args, func_default_args)
        if key_tuple in self._cache_nodes_hmap:
            ...
            new_cached_result = self.CachedResultNode(func_id, func_positional_args, func_default_args, func_result)
    ...
    def get_cached_result(self, func_id: str, func_positional_args: list[object], func_default_args: dict[object]) -> object:
        if self._max_size == 0:
            # Caching is disabled
            return None

        key_tuple = self._build_key_tuple(func_id, func_positional_args, func_default_args)
        if not self.check_cached(func_id, func_positional_args, func_default_args):
    ...
    def check_cached(self, func_id: str, func_positional_args: list[object], func_default_args: dict[object]) -> bool:
        if self._max_size == 0:
            return False
    ...
    def get_cache_dump(self) -> dict:
        ...
        def serialize_key(func_id, func_positional_args, func_default_args):
            return {
                'func_id': func_id,
        ...
        cache_dump = {}
        for (func_id, func_positional_args, func_default_args), node in self._cache_nodes_hmap.items():
            key_str = json.dumps(serialize_key(func_id, func_positional_args, func_default_args))
            cache_dump[key_str] = {
