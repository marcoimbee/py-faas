...
**Note that to successfully register a function, all type annotations must be provided, both for parameters and return type.**
...
If caching is enabled, functions' execution results can be cached at the worker: if a call to a previously registered function happens again with the same positional args - default args combination and such combination is stored in cache, the worker will not execute again such function, but will instead return directly the cache-extracted result to the client.
